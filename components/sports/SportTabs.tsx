...
export function SportTabs({
  ...
  return (
    <div className="space-y-3">
      <div className="-mx-1 flex gap-1.5 overflow-x-auto overflow-y-hidden pb-1 [-webkit-overflow-scrolling:touch]">
        {SPORTS.map((s) => (
          <button
            key={s.id}
            type="button"
            onClick={() => onChange({ ...filters, sport: s.id })}
            className={`shrink-0 rounded-lg border px-2.5 py-2 text-[10px] font-black uppercase tracking-wider transition ${
              sport === s.id
      ...
      <div className="-mx-1 flex gap-1 overflow-x-auto overflow-y-hidden border-b border-[#2A1D19] pb-2">
        {TABS.map((t) => (
          <button
            key={t.id}
            type="button"
            onClick={() => onChange({ ...filters, tab: t.id })}
            className={`shrink-0 rounded-md px-3 py-2 text-[11px] font-black uppercase tracking-wide ${
